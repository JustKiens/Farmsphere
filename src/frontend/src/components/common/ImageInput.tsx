...
import { useState, useCallback, useRef, DragEvent, ChangeEvent, MouseEvent } from 'react';
...
const ImageInput = ({ onChange, value, person }: ImageInputProps) => {
  ...
  return (
    <article className="flex items-center justify-start gap-6 w-full">
      <section
        className={`relative rounded-full ring-1 ${isDragging ? 'ring-green-500' : 'ring-gray-200'} w-24 h-24 flex items-center justify-center flex-shrink-0`}
      ...
      <p className="text-sm text-gray-500">For optimal results, please upload a square image of the {person}. A size of 
        <span className="text-green-500 font-medium mx-1">500px by 500px</span>with a
        <span className="text-green-500 font-medium mx-1">1:1</span> aspect ratio is recommended.
      </p>
      </section>
    </article>
  );
